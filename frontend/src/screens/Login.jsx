...
function LoginContent() {
  ...
  const complete = (data) => { persistSession(data); navigate(location.state?.from || '/', { replace: true }) }

  const submit = async (event) => {
    event.preventDefault(); setBusy(true)
    try {
      const response = await fetch(`${API_URL}/loginUser`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(credentials) })
      const data = await response.json()
      if (!response.ok || !data.success) throw new Error(data.error || 'Check your email and password')
      complete(data)
